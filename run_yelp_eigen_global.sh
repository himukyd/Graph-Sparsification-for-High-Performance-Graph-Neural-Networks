#!/bin/bash
#SBATCH --job-name=yelp_eigen_global
#SBATCH --partition=gpu
#SBATCH --gres=gpu:1
#SBATCH --time=12:00:00
#SBATCH --output=output_4/yelp_global_%j.out
#SBATCH --error=output_4/yelp_global_%j.err
...
echo "--- STARTING YELP GLOBAL EIGEN SPARSIFICATION EXPERIMENT ---"

# 1. Baseline: ORIGINAL Graph
echo "Step 1: Training GCN on Original Graph..."
python gcn_node_classification.py --dataset $DATASET --train_graph original --epoch $EPOCHS > ./output_4/yelp_original_global_${SLURM_JOB_ID}.txt
...
do
    ...
    if [ -f "$SPARSE_GRAPH" ]; then
        ...
        python gcn_node_classification.py --dataset $DATASET --train_graph $SPARSE_GRAPH --epoch $EPOCHS > ./output_4/yelp_eigen_p${P}_${SLURM_JOB_ID}.txt
    else
        echo "--> Error: Could not generate $SPARSE_GRAPH."
    fi
done

echo "--- YELP GLOBAL EIGEN EXPERIMENT COMPLETE ---"
